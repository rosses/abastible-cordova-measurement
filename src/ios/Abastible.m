...
- (void)startMeasurementMultiple:(CDVInvokedUrlCommand*)command {
    ...
    NSNumber* cylinderId = [command.arguments objectAtIndex:3];
    NSNumber* full = [command.arguments objectAtIndex:4];
    NSNumber* nrOfMeasurements = [command.arguments objectAtIndex:5];
    ...
    if(pluginResult != nil) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(measurementResult:) name:@"ABASTIBLE_MEASUREMENT_RESULT" object:nil];
        [Abastible_SDK.sharedInstance startMeasurement:ct tarra:[tarra intValue] fillingType:ft cilinderId:[cylinderId stringValue] cilinderIsFull:[full boolValue] nrOfValidMeasurements: [nrOfMeasurements intValue]];
    }
